...
<stylesheet xmlns="http://www.w3.org/1999/XSL/Transform" 
    ...
    <template match="*[@property and node()]">
	<call-template name="krextor:add-literal-property">
	    <with-param name="property" select="krextor:curies-to-uris(., @property)"/>
	    <with-param name="object" select="if (*)
		then ()
		else text()"/>
	    <!-- TODO implement XMLLiteral serialization (verb.xsl?) -->
	    <with-param name="object-datatype" select="if (*)
		then '&rdf;XMLLiteral'
		else ''"/>
	...
	<if test="@rel">
		...
		<with-param name="property" select="krextor:curies-to-uris(., @rel)"/>
		<with-param name="object" select="@href"/>
	    </call-template>
	</if>
	<if test="@rev">
	    <call-template name="krextor:add-uri-property">
